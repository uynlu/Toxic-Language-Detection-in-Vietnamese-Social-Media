...
class DataAnnotatorPipeline:
    def __init__(
        ...
        self.batch_folder = os.path.join(self.output_folder, "batches")
        os.makedirs(os.path.join(self.output_folder, "batches"), exist_ok=True)
        self.result_folder = os.path.join(self.output_folder, f"round_{self.prompt_round}")
    ...
    def main_parallelly(self):
        ...
        if self.optimization_flag:
            ...
            fixed_labelled_data = self.create_fixed_labelled_data(labelled_data, 100)
            os.makedirs(os.path.join(self.result_folder, "result"), exist_ok=True)
            save_json(fixed_labelled_data, os.path.join(self.result_folder, "result", f"{os.path.basename(self.output_folder)}_round_{self.prompt_round}_labelled_data_llms.json"))
        else:
            print("Start annotation process!")
            print("Start annotating!")
            self.annotate_parallelly()
