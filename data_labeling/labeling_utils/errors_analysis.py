...
def map_result(differently_labelled_path: str, result_folder: str, label_type: str):
    diff = load_json(differently_labelled_path)
    diff_mapping = {item["id"]: item["final"] for item in diff}

    for file_name in os.listdir(result_folder):
        if file_name.split("_")[-1] != "llms.json" and file_name.endswith(".json") and file_name != "result.json":
            data = load_json(os.path.join(result_folder, file_name))
            for item in data:
                if item["id"] in diff_mapping.keys():
                    item[f"{label_type}_fixed"] = diff_mapping.get(item["id"])
